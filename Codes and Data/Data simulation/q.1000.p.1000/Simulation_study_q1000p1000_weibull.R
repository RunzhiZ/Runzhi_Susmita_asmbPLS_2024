...
Parameter_Weibull <- Parameter_comb[Parameter_comb$Dist == "Weibull" & Parameter_comb$Censoring_rate != 0,]
...
# parameters setting
###############################################################################
Censoring_rate = Parameter_Weibull$Censoring_rate[arr_id]
Noise_level = Parameter_Weibull$Noise_level[arr_id]
Effect_scale = Parameter_Weibull$Effect_scale[arr_id]
...
quantile_2 <- c(0.9, 0.95, 0.975, 0.99, 0.995)
...
for(b in 1:6) {
  ...
  for(l in 1:niter) {
    ...
    Y_data <- Y_data_Weibull_list[[b]][[l]]
...
# Save results
###############################################################################
path <- paste0("/blue/datta/runzhi.zhang/asmbPLS/Sim202312/q.", q, ".p.", p,"/Sim_results_weibull/")
setwd(path)
